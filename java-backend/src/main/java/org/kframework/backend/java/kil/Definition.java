    ...
    public void addKoreRules(Module module, TermContext termContext) {
        ...
        JavaConversions.setAsJavaSet(module.sentences()).stream().forEach(s -> {
            if (s instanceof org.kframework.definition.Rule) {
                ...
                Location loc = rule.att().getOptional("org.kframework.attributes.Location", Location.class).orElse(null);
                Source source = rule.att().getOptional("org.kframework.attributes.Source", Source.class).orElse(null);
