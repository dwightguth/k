    ...
    private static void addNfaToGrammar(Sort sort, List<Production> prods, Grammar grammar, Nfa<Alphabet, Integer> minNfa, String rejectPattern) {
        ...
        for (Table.Cell<State<Integer>, Alphabet, Set<State<Integer>>> cell : minNfa.getTransitionFunction().cellSet()) {
            ...
            if (cell.getColumnKey() instanceof org.kframework.parser.NonTerminal) {
                ...
                states = Collections.singletonList(new Grammar.NonTerminalState(sort.name() + " := " + name, nt, grammar.get(name), false, cell.getColumnKey()));
                ...
                Grammar.RegExState state = new Grammar.RegExState(sort.name() + ": " + p, nt, p, cell.getColumnKey());
...
//                state.next.add(delState);
        ...
        //SetMultimap<State<Integer>, Production> exitStateMapping = computeMapOfExitStates(minNfa, prods);
...
//                    Pattern pattern = null;
                ...
                RuleState labelRule = new RuleState("AddLabelRS", nt, new WrapLabelRule(productionMap, prods.stream().collect(Collectors.toMap(p -> p, p -> Pattern.compile(getRejectPattern(p, rejectPattern))))));
                previous.next.add(labelRule);
                previous = labelRule;

                RuleState locRule = new RuleState("AddLocRS", nt, new AddLocationRule());
    ...
    private static String getRejectPattern(Production prd, String rejectPattern) {
        String pattern = "";
