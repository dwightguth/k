...
public class TstBackendOnKORE extends BaseTest {
    ...
    public void kore_csemantics() throws IOException, URISyntaxException {
        ...
        try {
            CompiledDefinition compiledDef = new Kompile(FileUtil.testFileUtil(), kem).run(new File(filename),
                    "C11-TRANSLATION", "C11-TRANSLATION", "K");
            ;
            K program = compiledDef.getProgramParser().apply("t(.Set, int) ==Type t(.Set, int)", Source.apply("generated by " + getClass().getSimpleName()));
            GetSymbolicRewriter getSymbolicRewriter = new GetSymbolicRewriter(compiledDef.getCompiledExecutionModule()).invoke();
