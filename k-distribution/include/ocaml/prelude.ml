...
sig
  type 'a m
  type s
  type t = KApply of klabel * t list
...
struct
  ...
  let rec compare c1 c2 = match (c1, c2) with
    ...
    | (KSequence k1), (KSequence k2) -> compare_klist k1 k2
...
let unwrap_kseq (k: kitem) : k list = match k with
| KSequence a -> a
| a -> a :: []

let wrap_kseq (k: k list) : kitem = match k with
| [k] -> k
| _ -> raise (Stuck (KSequence k))

let (@~>) : kitem -> kitem list -> kitem list = fun a b -> match a with
| KSequence a -> a @ b
| _ -> a :: b
...
| [] -> ".KList"
| e::[] -> print_kitem(e)
| e1::e2::l -> print_kitem(e1) ^ ", " ^ print_klist(e2::l)
...
| KSequence [] -> ".K"
| KSequence (e::[]) -> print_kitem e
| KSequence(e1::e2::l) -> print_kitem(e1) ^ " ~> " ^ print_kitem (KSequence(e2::l))
...
struct
  let hook_close c lbl sort config ff = match c with
      (Int i) :: [] -> Unix.close (Hashtbl.find file_descriptors i); KSequence []
    | _ -> raise Not_implemented
  ...
  let hook_putc c lbl sort config ff = match c with
      (Int fd) :: (Int c) :: [] -> let _ = Unix.write (Hashtbl.find file_descriptors fd) (Bytes.make 1 (Char.chr (Z.to_int c))) 0 1 in KSequence []
    | _ -> raise Not_implemented
  ...
  let hook_seek c lbl sort config ff = match c with
      (Int fd) :: (Int off) :: [] -> let o = (Z.to_int off) in let _ = Unix.lseek (Hashtbl.find file_descriptors fd) o Unix.SEEK_SET in KSequence []
    | _ -> raise Not_implemented
  ...
  let hook_write c lbl sort config ff = match c with
      (Int fd) :: (String s) :: [] -> let b = Bytes.of_string s in let _ = Unix.write (Hashtbl.find file_descriptors fd) b 0 (Bytes.length b) in KSequence []
    | _ -> raise Not_implemented
