...
public class MaudeKRun implements KRun {
    ...
    public static Term parseXML(Element xml, Context context) {
        ...
        try {
            if ((sort.equals(Sort.BAG_ITEM.getName()) || sort.equals("[Bag]")) && op.equals("<_>_</_>")) {
                Cell cell = new Cell();
                ...
                return cell;
            } else if ((sort.equals(Sort.BAG_ITEM.getName()) || sort.equals("[Bag]")) && op.equals(Sort.BAG_ITEM.getName())) {
                assertXMLTerm(list.size() == 1);
                return new BagItem(parseXML(list.get(0), context));
            } else if ((sort.equals(Sort.MAP_ITEM.getName()) || sort.equals("[Map]")) && op.equals("_|->_")) {
                assertXMLTerm(list.size() == 2);
                return MapBuiltin.element(mapSort, parseXML(list.get(0), context), parseXML(list.get(1), context));
            } else if ((sort.equals(Sort.SET_ITEM.getName()) || sort.equals("[Set]")) && op.equals(Sort.SET_ITEM.getName())) {
                assertXMLTerm(list.size() == 1);
                return SetBuiltin.element(setSort, parseXML(list.get(0), context));
            } else if ((sort.equals(Sort.LIST_ITEM.getName()) || sort.equals("[List]")) && op.equals(Sort.LIST_ITEM.getName())) {
                assertXMLTerm(list.size() == 1);
                ...
                return new KList(l);
            } else if (sort.equals(Sort.K.getName()) && op.equals("_~>_")) {
                assertXMLTerm(list.size() >= 2);
                List<Term> l = new ArrayList<Term>();
                for (Element elem : list) {
                    l.add(parseXML(elem, context));
            ...
            } else if (op.equals("__") && (sort.equals("NeList") || sort.equals(Sort.LIST.getName()) || sort.equals("[List]"))) {
                assertXMLTerm(list.size() >= 2);
                List<Term> l = new ArrayList<Term>();
                for (Element elem : list) {
                    l.add(parseXML(elem, context));
            ...
            } else if (op.equals("__") && (sort.equals("NeBag") || sort.equals(Sort.BAG.getName()) || sort.equals("[Bag]"))) {
                assertXMLTerm(list.size() >= 2);
                List<Term> l = new ArrayList<Term>();
                for (Element elem : list) {
                    l.add(parseXML(elem, context));
                ...
                return new Bag(l);
            } else if (op.equals("__") && (sort.equals("NeSet") || sort.equals(Sort.SET.getName()) || sort.equals("[Set]"))) {
                assertXMLTerm(list.size() >= 2);
                List<Term> l = new ArrayList<Term>();
                for (Element elem : list) {
                    l.add(parseXML(elem, context));
            ...
            } else if (op.equals("__") && (sort.equals("NeMap") || sort.equals(Sort.MAP.getName()) || sort.equals("[Map]"))) {
                assertXMLTerm(list.size() >= 2);
                List<Term> l = new ArrayList<Term>();
                for (Element elem : list) {
                    l.add(parseXML(elem, context));
            ...
            } else if ((op.equals("#_") || op.equals("Bag2KLabel_") || op.equals("KList2KLabel_") || op.equals("KLabel2KLabel_")) && (sort.equals(Sort.KLABEL.getName()) || sort.equals("[KLabel]"))) {
                assertXMLTerm(list.size() == 1);
            ...
            } else if (op.equals("#token") && sort.equals(Sort.KLABEL.getName())) {
                // #token(String, String)
                assertXMLTerm(list.size() == 2);
            ...
            } else if (emptyPattern.matcher(op).matches() && (sort.equals(Sort.BAG.getName())
                    || sort.equals(Sort.LIST.getName())
                    || sort.equals(Sort.MAP.getName())
                    || sort.equals(Sort.SET.getName())
                    || sort.equals(Sort.K.getName()))) {
                assertXMLTerm(list.size() == 0);
                if (sort.equals(Sort.BAG.getName())) {
                    return Bag.EMPTY;
                } else if (sort.equals(Sort.LIST.getName())) {
                    return DataStructureBuiltin.empty(listSort);
                } else if (sort.equals(Sort.MAP.getName())) {
                    return DataStructureBuiltin.empty(mapSort);
                } else if (sort.equals(Sort.SET.getName())) {
                    return DataStructureBuiltin.empty(setSort);
                } else {
                    // sort.equals(KSorts.K)
                    return KSequence.EMPTY;
                }
            } else if (op.equals(".KList") && sort.equals(Sort.KLIST.getName())) {
                assertXMLTerm(list.size() == 0);
                return KList.EMPTY;
            } else if (op.equals("_`(_`)") && (sort.equals(Sort.KITEM.getName()) || sort.equals("[KList]"))) {
                assertXMLTerm(list.size() == 2);
            ...
            } else if (sort.equals(Sort.KLABEL.getName()) && list.size() == 0) {
                return KLabelConstant.of(StringUtil.unescapeMaude(op), context);
            } else if (sort.equals(Sort.KLABEL.getName()) && op.equals("#freezer_")) {
                assertXMLTerm(list.size() == 1);
                return new FreezerLabel(parseXML(list.get(0), context));
            } else if (op.equals("HOLE")) {
                assertXMLTerm(list.size() == 0 && sort.equals(Sort.KITEM.getName()));
